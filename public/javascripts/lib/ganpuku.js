...
(function($){
    ...
    var YouTube = function(id, params){
        ...
        var that = this;
        this.player.addEventListener('onReady', function(ev){
          if(that.player) that.fire('ready', that.player);
        });
    ...
    $.fn.ganpuku = function(url, options){
        if(false == api.is_loaded) api.load();
        if(ganpuku != null){
          console.log(ganpuku.player.destroy)
          ganpuku = null;
        }
        if($(this).attr('id')){
          ganpuku = new Ganpuku(this, url, options)
        }
